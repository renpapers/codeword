...
//arrays source: Dan Shiffman 
//galaxy source: https://js6450.github.io/
...
var sentenceCount = 0;
var maxGalaxies = 1084; // 1Q84
var galaxies = [];
var letterCount = 0;
var letters = 'Four Horses Cannot Draw it in'; //Translated: Yi Hua Ji Chu Si Nan Zhui - Once something is said, it cannot be unsaid. 
var song;



function preload(){
  font = loadFont('data/Sicret.ttf');
  song = loadSound('data/music.mp3'); //'The Star Crossed Lovers' by Duke Ellington 
}
function setup() {
...
text('left click = creating galaxies', windowWidth/56, windowHeight/14);
...
function keyPressed(){
  if(key == " "){
    for(var i = 0; i < galaxies.length; i++){
      galaxies[i].speedX = random(-5, 5) * 3;
      galaxies[i].speedY = random(-5, 5) * 3;
    }
  }
} 
...
function Galaxy(x, y){
  ...
  this.textSize = size;
  this.color = color(255, 254, 242, 100);
  this.letter = letter;
